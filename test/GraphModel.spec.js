...
let dimensionalData = new DimensionalData(uniques.desktop.items.concat(uniques.mobile.items));
let graphModel = null;

describe('GraphModel', function () {
    beforeEach(() => {
        graphModel = new GraphModel('es.wikipedia.org', 'unique-devices');
        graphModel.dimensions = Dimension.fromMetricConfig(metric);
    ...
    it('should aggregate average when metric is not additive', function () {
        const metricConfig = Object.assign(metric, {additive: false});
        graphModel.config = metricConfig;
